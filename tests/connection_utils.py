...
async def disconnect_all_and_reconnect(server: DeafwaveServer, reconnect_to: DeafwaveServer) -> bool:
...
async def add_dummy_connection(server: DeafwaveServer, dummy_port: int) -> Tuple[asyncio.Queue, bytes32]:
    ...
    generate_ca_signed_cert(
        ...
        server.deafwave_ca_key_path.read_bytes(),
        dummy_crt_path,
        dummy_key_path,
    )
...
async def connect_and_get_peer(server_1: DeafwaveServer, server_2: DeafwaveServer) -> WSDeafwaveConnection:
    """
