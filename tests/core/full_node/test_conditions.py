...
class TestConditions:
    ...
    async def test_invalid_my_id(self):
        ...
        wrong_name[-1] ^= 1
        conditions = Program.to(
            assemble(f"(({ConditionOpcode.ASSERT_MY_COIN_ID[0]} 0x{wrong_name.hex()}))"))
    ...
    async def test_valid_my_id(self):
        ...
        coin = list(blocks[-2].get_included_reward_coins())[0]
        conditions = Program.to(
            assemble(f"(({ConditionOpcode.ASSERT_MY_COIN_ID[0]} 0x{coin.name().hex()}))"))
        await check_conditions(conditions)
    ...
    async def test_invalid_seconds_absolute(self):
        ...
        conditions = Program.to(
            assemble(f"(({ConditionOpcode.ASSERT_SECONDS_ABSOLUTE[0]} {time_now}))"))
